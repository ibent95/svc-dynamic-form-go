...
func main() {
	...
	log.Println("🔍 Listing all parsed components and their dependencies:")
	for i, c := range components {
		log.Printf("   %d. %s: %v", i + 1, c.Name, c.Deps)
	}
...
func canAddComponent(comp Component, added map[string]bool) bool {
	...
	for _, dep := range comp.Deps {
		...
		// Skip the DB dependency as it is assumed to be globally injected from configs
		if dep == "DB" {
			continue
		}

		// If the dependency is not in the added map, we cannot add this component yet
		if !added[dep] {
			return false
		}
	}
